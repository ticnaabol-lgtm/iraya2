...
@section('content')
    <div class="px-lg py-lg bg-card">
        ...
        <h4>Autorizacion Ingreso / Salida</h4>
                ...
                <tbody>
                    @foreach ($autorizaciones as $item)
                        <tr>
                            ...
                            <td>{{ $item->aeropuerto_entrada }}</td>
                            <td>{{ $item->aeropuerto_salida }}</td>
                            ...
                            <td>
                                ...
                                <button class="btn btn-info btn-icon rounded-circle btn-detalles-autorizacion"
                                    ...
                                    data-aeropuerto_entrada="{{ $item->aeropuerto_entrada }}"
                                    data-aeropuerto_entrada_alterno="{{ $item->aeropuerto_entrada_alterno }}"
                                    data-aeropuerto_salida="{{ $item->aeropuerto_salida }}"
                                    data-aeropuerto_salida_alterno="{{ $item->aeropuerto_salida_alterno }}"
    ...
    $(document).ready(function() {
        ...
        socket.on('registro_autorizacion', function(data) {
            ...
            data.forEach(function(item, index) {
                // Filtrar solo los que NO sean "SOBREVUELO"
                if (item.tipo_autorizacion !== "SOBREVUELO") {
                    ...
                    table.row.add([
                        ...
                        limitarTexto(item.aeropuerto_entrada),
                        limitarTexto(item.aeropuerto_salida),
                        limitarTexto(item.ruta),
                        limitarTexto(item.tipo_autorizacion),
