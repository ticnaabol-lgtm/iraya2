...
class TransitoAereoController extends Controller
{
    ...
    public function obtenerAmhsPag(Request $request)
    {
        try {
            ...
            try {
                ...
                // Obtener los registros limitados
                /* $query = DB::table('fpls')
    
                    ->leftJoin('proceso_vuelo', function ($join) use ($request) {
                        $join->on('fpls.id', '=', 'proceso_vuelo.id_amhs');
    
                        // Agregar condición dinámica para tipo_ficha
                        if ($request->filled('tipo_ficha')) {
                            $join->where('proceso_vuelo.tipo_ficha', '=', $request->input('tipo_ficha'));
                        }
                    })
                    ->leftJoin('users', 'proceso_vuelo.fk_user', '=', 'users.id') // Join con users
                ...
                // Obtener todos los ids de amhs para evitar N+1 queries
                // $amhsIds = $amhs->pluck('id');

                // Obtener todos los registros de proceso_vuelo y FichaEstado en una sola consulta
    ...
    public function buscador_amhs(Request $request)
    {
        ...
        try {
            ...
            // Hacer la solicitud GET con los parámetros inicio y fin
            $response2 = $servicio2->request('POST', '/api/amhs-search', [
                'query' => [
                    'inicio' => $inicio,
                    'fin' => $fin,
    ...
    /* public function store(Request $request)
    {
        ...
        try {
            ...
            $response = Http::post(env('TRANSITO_API_URL') . '/api/ficha_progreso', [
            ...
            if ($response->successful()) {
                ...
                $createdId = $responseData['id'] ?? null;

                return response()->json([
                    'message' => 'Ficha de progreso creada con éxito',
                    'id' => $createdId,
                    'data' => $responseData
                ], 201); // Código de estado 201 Created
            } else {
                Log::error('Error al crear ficha de progreso: ' . $response->body());
                return response()->json(['error' => 'Error-01 al crear la ficha de progreso'], 500); // Código de estado 500 Internal Server Error
            }
        } catch (\Exception $e) {
            Log::error('Error al crear ficha de progreso: ' . $e->getMessage());
            return response()->json(['error' => 'Error-02 al crear la ficha de progreso'], 500); // Código de estado 500 Internal Server Error
        }
    } */
